...
var DASH_DIRECTION #ОТ этой надо избавится
...
func _physics_process(delta):
	match state:
		...
		DASH:
			dash_state(delta, DASH_DIRECTION)
			get_action_input(delta)
	print(velocity)
########################
...
func move_character(delta):
	if get_move_direction(delta) != Vector2.ZERO:
		#velocity += get_move_direction(delta) * ACCELERATION * delta 
		#velocity = velocity.clamped(MAX_SPEED)
		velocity = velocity.move_toward(MAX_SPEED * get_move_direction(delta), ACCELERATION)
		#velocity = velocity.linear_interpolate(get_move_direction(delta)* MAX_SPEED , FRICTION)
	else:
		velocity = velocity.linear_interpolate(Vector2.ZERO, FRICTION)
	velocity = move_and_slide(velocity)

#Двигаем спрайт со скоростью дэша
func dash_state(delta, move_direction):
